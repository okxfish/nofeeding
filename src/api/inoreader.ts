import { fetch, INOREADER_BASE_URL, CORS_PROXY_URL } from "./index";

const inoreaderToken = localStorage.getItem("inoreaderToken") || "";

const SystemStreamIDs = {
...
export const inoreader = {
  getSubscriptionList: () =>
    fetch.get(`/reader/api/0/subscription/list`, {
      baseURL: `${CORS_PROXY_URL}/${INOREADER_BASE_URL}`,
      headers: { Authorization: `Bearer ${inoreaderToken}` },
    }),

  getStreamContents: () =>
    fetch.get(`/reader/api/0/stream/contents`, {
      params: {
        ...
        it: '', // nclude Target - You can query for a certain label with this. Accepted values: user/-/state/com.google/starred, user/-/state/com.google/like.
        ...
        streamId: '', // is not a parameter. It is appended to the URL, but it's important to URL encode it first.
      },
      baseURL: `${CORS_PROXY_URL}/${INOREADER_BASE_URL}`,
      headers: { Authorization: `Bearer ${inoreaderToken}` },
    }),
  ...
  getUserInfo: () =>
    fetch.get(`/reader/api/0/user-info`, {
      baseURL: `${CORS_PROXY_URL}/${INOREADER_BASE_URL}`,
      headers: { Authorization: `Bearer ${inoreaderToken}` },
    }),
